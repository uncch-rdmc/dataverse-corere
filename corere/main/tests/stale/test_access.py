...
class TestUrls(TestCase):
    ...
    def test_manuscript_urls_not_logged_in(self):
        ...
        resp = self.client.get(
            reverse("manuscript_unassigncurator", kwargs={"id": self.manuscript.id + 1, "user_id": self.user2.id})
        )  # id we know hasn't been created
        self.assertEqual(resp.status_code, 302)
        resp = self.client.get(reverse("manuscript_unassignverifier", kwargs={"id": self.manuscript.id, "user_id": self.user2.id}))
        self.assertEqual(resp.status_code, 302)
        resp = self.client.get(
            reverse("manuscript_unassignverifier", kwargs={"id": self.manuscript.id + 1, "user_id": self.user2.id})
        )  # id we know hasn't been created
        self.assertEqual(resp.status_code, 302)
        resp = self.client.get(reverse("manuscript_createsubmission", kwargs={"manuscript_id": self.manuscript.id}))
        self.assertEqual(resp.status_code, 302)
        resp = self.client.get(
            reverse("manuscript_createsubmission", kwargs={"manuscript_id": self.manuscript.id + 1})
        )  # id we know hasn't been created
        self.assertEqual(resp.status_code, 302)
    ...
    def test_manuscript_urls_logged_in_multiple_roles(self, mock_gitlab_file_list):
        ...
        Group.objects.get(name=c.GROUP_MANUSCRIPT_EDITOR_PREFIX + " " + str(self.manuscript.id)).user_set.add(
            self.user
        )  # test user is editor on manuscript 1
        Group.objects.get(name=c.GROUP_ROLE_CURATOR).user_set.add(self.user)  # test user is curator
        Group.objects.get(name=c.GROUP_MANUSCRIPT_CURATOR_PREFIX + " " + str(self.manuscript.id)).user_set.add(
            self.user
        )  # test user is curator on manuscript 1
        Group.objects.get(name=c.GROUP_ROLE_AUTHOR).user_set.add(self.user)  # test user is authorr
        Group.objects.get(name=c.GROUP_MANUSCRIPT_AUTHOR_PREFIX + " " + str(self.manuscript.id)).user_set.add(
            self.user
        )  # test user is author on manuscript 1
    ...
    def test_submission_curation_verification_urls_not_logged_in(self):
        ...
        self.assertEqual(resp.status_code, 302)
        local.user = (
            None  # accessing pages sets local.user=AnonymousUser which blows up our save tracking (anon users will never be creating in real runs)
        )
        ...
        self.assertEqual(resp.status_code, 302)
        resp = self.client.get(
            reverse("submission_createverification", kwargs={"submission_id": submission.id + 1})
        )  # id we know hasn't been created
        self.assertEqual(resp.status_code, 302)
    ...
    def test_submission_curation_verification_urls_logged_in_all_roles(self, mock_gitlab_file_list):
        ...
        Group.objects.get(name=c.GROUP_MANUSCRIPT_VERIFIER_PREFIX + " " + str(self.manuscript.id)).user_set.add(
            self.user
        )  # test user is author on manuscript 1
