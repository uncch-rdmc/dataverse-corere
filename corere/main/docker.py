...
def build_repo2docker_image(manuscript):
    ...
    image_name = (
        ("jupyter-" + str(manuscript.id) + "-" + manuscript.slug + "-version" + str(sub_version))[:128]
        + ":"
        + settings.DOCKER_GEN_TAG
        + "-"
        + str(manuscript.id)
    )
...
def _write_oauthproxy_email_list_to_working_directory(manuscript):
    ...
    user_email_list = m.User.objects.filter(
        Q(groups__name__startswith=c.GROUP_MANUSCRIPT_AUTHOR_PREFIX + " " + str(manuscript.id))
...
def build_oauthproxy_image(manuscript):
    ...
    docker_string = (
        "FROM "
        + settings.DOCKER_OAUTH_PROXY_BASE_IMAGE
        + "\n"
    ...
    container_info.proxy_image_name = (
        ("oauthproxy-" + str(manuscript.id) + "-" + manuscript.slug)[:128] + ":" + settings.DOCKER_GEN_TAG + "-" + str(manuscript.id)
    )
    container_info.save()
...
def start_repo2docker_container(manuscript):
    ...
    run_string = (
        ...
        + container_info.container_public_address()
        + "' "
    ...
    notebook_network.disconnect(
        container, force=True
    )  # we disconnect it and then re-add it with the correct ip. I couldn't find a way to start the contain with no network and then just add this.
...
def start_oauthproxy_container(manuscript, request):
    ...
    command = (
        ...
        + "--upstream="
        + "'"
        + settings.CONTAINER_PROTOCOL
        + "://"
        + settings.CONTAINER_TO_CORERE_ADDRESS
        + "/submission/"
        ...
        + "--cookie-secret="
        + "'"
        + settings.OAUTHPROXY_COOKIE_SECRET
        + "'"
        + " "
        + "--cookie-refresh="
        + "'0s'"
        ...
        + "--banner="
        + "'"
        + "Please re-authenticate to access the environment for Manuscript: "
        + manuscript.get_display_name()
        + "'"
        + " "
        + "--footer="
        + "'"
...
def start_network(manuscript):
    ...
    ipam_pool = docker.types.IPAMPool(subnet=network_sub + ".0/16", gateway=network_sub + ".1")
