...
def label_gen(model, field_strings, required=[]):
    ...
    for field_string in field_strings:
        ...
        if field.help_text != "":
            html += (
                '<span class="fas fa-question-circle tooltip-icon" data-toggle="tooltip" data-placement="auto" title="'
                + field.help_text
                + '"></span>'
            )
...
class AuthorInviteAddForm(forms.Form):
    ...
    users_to_add = ModelMultipleChoiceField(
        label="Select Existing CoRe2 User",
        queryset=m.User.objects.filter(invite__isnull=True, groups__name=c.GROUP_ROLE_AUTHOR),
        widget=CustomSelect2UserWidget(),
        required=False,
    )

    def clean(self):
        if (
            self.cleaned_data.get("first_name", None) and self.cleaned_data.get("last_name", None) and self.cleaned_data.get("email", None)
...
class EditorAddForm(forms.Form):
    users_to_add = ModelMultipleChoiceField(
        queryset=m.User.objects.filter(invite__isnull=True, groups__name=c.GROUP_ROLE_EDITOR), widget=CustomSelect2UserWidget(), required=False
    )


class CuratorAddForm(forms.Form):
    users_to_add = ModelMultipleChoiceField(
        queryset=m.User.objects.filter(invite__isnull=True, groups__name=c.GROUP_ROLE_CURATOR), widget=CustomSelect2UserWidget(), required=False
    )


class VerifierAddForm(forms.Form):
    users_to_add = ModelMultipleChoiceField(
        queryset=m.User.objects.filter(invite__isnull=True, groups__name=c.GROUP_ROLE_VERIFIER), widget=CustomSelect2UserWidget(), required=False
    )
...
class ManuscriptBaseForm(forms.ModelForm):
    ...
    class Meta:
        ...
        always_required = [
            ...
            "contact_last_name",
            "contact_email",
        ]  # Used to populate required "*" in form. We have disabled the default crispy functionality because it isn't dynamic enough for our per-phase requirements
        labels = label_gen(model, fields, always_required)

    compute_env = forms.ModelChoiceField(
        queryset=wtm.ImageChoice.objects.filter(hidden=False), empty_label=None, required=False, label="Compute Environment"
    )
    ...
    def save(self, commit=True, *args, **kwargs):
        ...
        if wt_id and not re.match("^[\w\d]*$", wt_id):
            logger.warning(
                "Someone attempted attempted to set compute_env id:{1} to an invalid string. They may have tried hacking the form.".format(
                    self.instance.id
                )
            )
            raise Http404()
    ...
    def clean(self):
        ...
        if "submit_progress_manuscript" in self.data.keys() or self.instance._status != m.Manuscript.Status.NEW:
            ...
            if (
                self.data["author_formset-0-first_name"] == ""
                or self.data["author_formset-0-last_name"]
                == ""  # or self.data['author_formset-0-identifier'] == "" or self.data['author_formset-0-identifier_scheme'] == ""
            ):
            ...
            if not (
                self.instance._status != m.Manuscript.Status.PENDING_DATAVERSE_PUBLISH
                or self.instance._status != m.Manuscript.Status.PUBLISHED_TO_DATAVERSE
...
class NoteForm(forms.ModelForm):
    ...
    scope = forms.ChoiceField(widget=forms.RadioSelect, choices=SCOPE_OPTIONS, required=False)
    ...
    def __init__(self, *args, checkers, manuscript, submission, sub_files, **kwargs):
        ...
        # user = CrequestMiddleware.get_request().user
        path_obj_name = CrequestMiddleware.get_request().resolver_match.url_name.split("_")[
            0
        ]  # CrequestMiddleware.get_request().resolver_match.func.view_class.object_friendly_name
        ...
        if not curator_verifier:
            ...
            if user.has_any_perm(c.PERM_MANU_ADD_AUTHORS, manuscript):  # Check for editor
                self.fields["creator"].widget.choices = [
                    ("Curator", "Curator"),
                    ("Verifier", "Verifier"),
                    ("Editor", "Editor"),
                    ("Author", "Author"),
    ...
    def save(self, commit, *args, **kwargs):
        if self.has_changed():
            ...
            if not had_id or "scope" in self.changed_data:
                ...
                if (
                    not "scope" in self.cleaned_data or self.cleaned_data["scope"] == "public"
                ):  # Scope isn't in the form for author/editors, which defaults to public
                    for role in c.get_roles():
                        group = Group.objects.get(name=role)
                        assign_perm(c.PERM_NOTE_VIEW_N, group, self.instance)
                else:
                    if user.has_any_perm(c.PERM_MANU_CURATE, self.instance.manuscript) or user.has_any_perm(
                        c.PERM_MANU_VERIFY, self.instance.manuscript
                    ):  # only users with certain roles can set private
                        ...
                        logger.warning(
                            "User id:{0} attempted to set note id:{1} to private, when they do not have the required permissions. They may have tried hacking the form.".format(
...
class BaseFileNestingFormSet(BaseInlineFormSet):
    def add_fields(self, form, index):
        ...
        form.nested = self.nested_formset(
            ...
            prefix="note-%s-%s" % (form.prefix, self.nested_formset.get_default_prefix()),  # Defining prefix seems nessecary for getting save to work
        )
...
class DownloadGitFileWidget(forms.widgets.TextInput):
    ...
    def get_context(self, name, value, attrs):
        try:
            self.download_url = (
                "/submission/"
                + str(self.object_instance.parent_submission.id)
                + "/downloadfile/?file_path="
                + self.object_instance.path
                + self.object_instance.name
            )
...
GitFileFormSet = inlineformset_factory(
    ...
    extra=0,
    can_delete=False,
    widgets={"name": DownloadGitFileWidget(), "description": Textarea(attrs={"rows": 1, "class": "shortarea"})},
)
