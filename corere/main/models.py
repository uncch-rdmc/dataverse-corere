...
class AbstractCreateUpdateModel(models.Model):
    ...
    creator = models.ForeignKey(
        "User", on_delete=models.SET_NULL, related_name="creator_%(class)ss", blank=True, null=True, verbose_name="Creator User"
    )
    last_editor = models.ForeignKey(
        "User", on_delete=models.SET_NULL, related_name="last_editor_%(class)ss", blank=True, null=True, verbose_name="Last Updating User"
    )
...
class Edition(AbstractCreateUpdateModel):
    ...
    @transition(
        ...
        permission=lambda instance, user: (
            (
                instance.submission._status == Submission.Status.IN_PROGRESS_EDITION
                and user.has_any_perm(c.PERM_MANU_APPROVE, instance.submission.manuscript)
            )
            or (
                instance.submission._status != Submission.Status.IN_PROGRESS_EDITION
                and user.has_any_perm(c.PERM_MANU_VIEW_M, instance.submission.manuscript)
            )
        ),
    )
    def view_noop(self):
...
class Curation(AbstractCreateUpdateModel):
    ...
    @transition(
        ...
        permission=lambda instance, user: (
            (
                instance.submission._status == Submission.Status.IN_PROGRESS_CURATION
                and user.has_any_perm(c.PERM_MANU_CURATE, instance.submission.manuscript)
            )
            or (
                instance.submission._status != Submission.Status.IN_PROGRESS_CURATION
                and user.has_any_perm(c.PERM_MANU_VIEW_M, instance.submission.manuscript)
...
class Verification(AbstractCreateUpdateModel):
    ...
    @transition(
        ...
        or user.has_any_perm(c.PERM_MANU_CURATE, instance.submission.manuscript),
    )
    def edit_noop(self):
        return self._status
    ...
    @transition(
        ...
        permission=lambda instance, user: (
            (
                instance.submission._status == Submission.Status.IN_PROGRESS_VERIFICATION
                and user.has_any_perm(c.PERM_MANU_VERIFY, instance.submission.manuscript)
            )
            or (
                instance.submission._status != Submission.Status.IN_PROGRESS_VERIFICATION
                and user.has_any_perm(c.PERM_MANU_VIEW_M, instance.submission.manuscript)
...
class Submission(AbstractCreateUpdateModel):
    ...
    launch_issues = models.TextField(
        max_length=1024,
        blank=True,
        null=True,
        default="",
        verbose_name="Container Launch Issues",
        help_text="Issues faced when attempting to launch the container",
    )
    editor_submit_date = models.DateField(
        verbose_name="Editor Submit Date",
        blank=True,
        null=True,
        help_text="The date when editors submitted the submission. Only used when editor use of CORE2 for a manuscript is disabled.",
    )
    ...
    def save(self, *args, **kwargs):
        ...
        if not self.pk:  # only first save. Nessecary for submission in progress check but also to allow admin editing of submissions
            ...
            try:
                ...
                if (
                    self.manuscript._status != Manuscript.Status.AWAITING_INITIAL
                    and self.manuscript._status != Manuscript.Status.AWAITING_RESUBMISSION
                ):
            ...
            if self.version_id > 1:
                ...
                if self.manuscript.is_containerized() and settings.CONTAINER_DRIVER == "wholetale":
                    for (
                        ...
                        tale.tale_copies.all()
                    ):  # delete copy instances from previous submission. Note this happens as admin from the previous connection above
                        wtc.delete_tale(tale.wt_id)  # deletes instances as well
        elif (
            self._status == self.Status.REJECTED_EDITOR and self.manuscript.is_containerized() and settings.CONTAINER_DRIVER == "wholetale"
        ):  # If editor rejects we need to give the author write access again to the same submission
            wtc = w.WholeTaleCorere(admin=True)
            tale = self.manuscript.manuscript_tales.get(original_tale=None)
            group = Group.objects.get(name__startswith=c.GROUP_MANUSCRIPT_AUTHOR_PREFIX + " " + str(self.manuscript.id))
            wtc.set_group_access(tale.wt_id, wtc.AccessType.WRITE, group.wholetale_group)
    ...
    @transition(
        ...
        conditions=[],
        permission=lambda instance, user: (
            (instance._status == instance.Status.NEW or instance._status == instance.Status.REJECTED_EDITOR)
            and user.has_any_perm(c.PERM_MANU_ADD_SUBMISSION, instance.manuscript)
    ...
    @transition(
        ...
        permission=lambda instance, user: (
            ...
            or (
                instance._status != instance.Status.NEW
                and instance._status != instance.Status.REJECTED_EDITOR
                and user.has_any_perm(c.PERM_MANU_VIEW_M, instance.manuscript)
            )
        ),
    )
    def view_noop(self):
    ...
    @transition(
        field=_status,
        source=[Status.IN_PROGRESS_EDITION],
        target=RETURN_VALUE(),
        conditions=[can_add_edition],
    ...
    @transition(
        field=_status,
        source=[Status.IN_PROGRESS_CURATION],
        target=RETURN_VALUE(),
        conditions=[can_add_curation],
    ...
    @transition(
        field=_status,
        source=[Status.IN_PROGRESS_VERIFICATION],
        target=RETURN_VALUE(),
        conditions=[can_add_verification],
    ...
    @transition(
        ...
        source=[Status.IN_PROGRESS_CURATION],
        target=RETURN_VALUE(),
        conditions=[can_review_curation],
        permission=lambda instance, user: (user.has_any_perm(c.PERM_MANU_CURATE, instance.manuscript)),
    )
    def review_curation(self):
        ...
        if not settings.SKIP_GIT:
            g.create_submission_branch(
                self
            )  # We create the submission branch before returning the submission, to "save" the current state of the repo for history
    ...
    @transition(
        ...
        source=[Status.IN_PROGRESS_VERIFICATION],
        target=RETURN_VALUE(),
        conditions=[can_review_verification],
        permission=lambda instance, user: (user.has_any_perm(c.PERM_MANU_VERIFY, instance.manuscript)),
    )
    ...
    def can_send_report(self):
        if (
            ...
            or self.manuscript._status == Manuscript.Status.PENDING_DATAVERSE_PUBLISH
        ):
            return False
        else:
            return True
    ...
    @transition(
        field=_status,
        source=Status.REVIEWED_AWAITING_REPORT,
        target=RETURN_VALUE(),
        conditions=[can_send_report],
        permission=lambda instance, user: (user.has_any_perm(c.PERM_MANU_CURATE, instance.manuscript)),
    )
    ...
    @transition(
        field=_status,
        source=Status.REVIEWED_REPORT_AWAITING_APPROVAL,
        target=Status.RETURNED,
        conditions=[can_finish_submission],
        permission=lambda instance, user: (user.has_any_perm(c.PERM_MANU_APPROVE, instance.manuscript)),
    )
...
class Manuscript(AbstractCreateUpdateModel):
    class Status(models.TextChoices):
        ...
        PENDING_DATAVERSE_PUBLISH = (
            "pending_dataverse_publish",
            "Pending Dataverse Publish",
        )  # TODO: Maybe rename this to "Approved?". Technically its not completed when this is set because we still need to send the report and upload to dataverse
    ...
    pub_id = models.CharField(
        max_length=200, default="", db_index=True, verbose_name="Manuscript #", help_text="The internal ID from the publication"
    )
    ...
    qdr_review = models.BooleanField(
        default=False, verbose_name="Does this manuscript need verification of qualitative results by QDR?", help_text="TODO"
    )
    contact_first_name = models.CharField(
        max_length=150,
        verbose_name="Corresponding Author Given Name",
        help_text="Given name of the publication contact that will be stored in Dataverse",
    )
    contact_last_name = models.CharField(
        max_length=150, verbose_name="Corresponding Author Surname", help_text="Surname of the publication contact that will be stored in Dataverse"
    )
    contact_email = models.EmailField(
        null=True,
        verbose_name="Corresponding Author Email Address",
    ...
    compute_env = models.CharField(
        max_length=100, blank=True, null=True, verbose_name="Compute Environment Format"
    )  # This is set to longer than 24 to bypass a validation check due to form weirdness. See the manuscript form save function for more info
    compute_env_other = models.TextField(
        max_length=1024,
        blank=True,
        null=True,
        default="",
        verbose_name="Other Environment Details",
        help_text="Details about the unlisted environment",
    )

    # Was a part of submission
    high_performance = models.BooleanField(
        default=False, verbose_name="Does this manuscript require a high-performance compute environment?", help_text="TODO"
    )
    ...
    contents_restricted = models.BooleanField(
        default=False, verbose_name="Does this manuscript contain restricted or proprietary data?", help_text="TODO"
    )
    contents_restricted_sharing = models.BooleanField(
        default=False, verbose_name="Are you restricted from sharing this data with Odum for verification only?", help_text="TODO"
    )
    other_exemptions = models.TextField(
        max_length=1024,
        blank=True,
        null=True,
        default="",
        verbose_name="Other Exemptions",
        help_text="Are there any other exemptions to the verification workflow that the curation team should know about?",
    )

    exemption_override = models.BooleanField(
        default=False,
        verbose_name="Exemption Override",
        help_text="The curation team has decided to deploy this manuscript inside Whole Tale, even with potential issues.",
    )

    operating_system = models.CharField(max_length=200, default="", verbose_name="Operating System")
    packages_info = models.TextField(
        blank=True,
        null=True,
        default="",
    ...
    )  # currently only used for naming a file folder on upload. Needed as id doesn't exist until after create
    history = HistoricalRecords(
        bases=[
            AbstractHistoryWithChanges,
        ],
    ...
    dataverse_parent = models.CharField(
        max_length=1024,
        blank=True,
        null=True,
        default="",
        verbose_name="Parent Dataverse",
        help_text="The parent Dataverse on the installation that your new dataset will be created under. Please provide the name at the end of the dataverse page URL (e.g. https://dataverse.unc.edu/dataverse/COPY_THIS_NAME)",
    )
    dataverse_installation = models.ForeignKey(
        "DataverseInstallation",
        blank=True,
        null=True,
        verbose_name="Dataverse Installation",
        on_delete=models.SET_NULL,
        related_name="dataverseinstallation_manuscripts",
    )
    dataverse_fetched_doi = models.CharField(
        max_length=150, blank=True, verbose_name="Dataverse DOI", help_text="DOI of the publication in Dataverse"
    )
    dataverse_fetched_data_citation = models.TextField(
        default="",
        blank=True,
        null=True,
        verbose_name="Dataverse Data Citation",
        help_text="The data citation pulled from the dataset connected to this manuscript (via DOI)",
    )
    dataverse_fetched_article_citation = models.TextField(
        default="",
        blank=True,
        null=True,
        verbose_name="Dataverse Article Citation",
        help_text="The article citation pulled from the dataset connected to this manuscript (via DOI)",
    )
    dataverse_fetched_publish_date = models.DateField(
        verbose_name="Dataset Publish Date", blank=True, null=True, help_text="The date the dataset in Dataverse was published"
    )
    ...
    def save(self, *args, **kwargs):
        ...
        if first_save:
            ...
            group_manuscript_editor.user_set.add(
                local.user
            )  # TODO: Should be dynamic on role or more secure, but right now only editors create manuscripts. Will need to fix wt invite below as well.
        ...
        if settings.CONTAINER_DRIVER == "wholetale" and self.is_containerized() and not self.manuscript_tales.all().exists():
            ...
            if not first_save:
                try:  # All this code is for recreating a tale after an environment switch.
                    ...
                    tale.save()
                    wtc.upload_files(
                        tale.wt_id, g.get_submission_files_path(self)
                    )  # Upload existing files. Only should actually do anything when is_containerized changes because we check for tale existence.
    ...
    def is_containerized(self):
        if (
            ...
            or ((self.high_performance or self.contents_restricted_sharing) and not self.exemption_override)
        ):
            return False
        return True
    ...
    @transition(
        field=_status,
        source=Status.NEW,
        target=Status.AWAITING_INITIAL,
        conditions=[can_begin],
    ...
    def can_add_submission(self):
        ...
        if (
            self.manuscript_submissions.filter(
                Q(_status=Submission.Status.NEW)
                | Q(_status=Submission.Status.REJECTED_EDITOR)
                | Q(_status=Submission.Status.IN_PROGRESS_EDITION)
                | Q(_status=Submission.Status.IN_PROGRESS_CURATION)
                | Q(_status=Submission.Status.IN_PROGRESS_VERIFICATION)
            ).count()
            != 0
        ):
            return False
    ...
    @transition(
        field=_status,
        source=[Status.AWAITING_INITIAL, Status.AWAITING_RESUBMISSION],
        target=RETURN_VALUE(),
        conditions=[can_add_submission],
    ...
    @transition(
        ...
        conditions=[],
        permission=lambda instance, user: (
            user.has_any_perm(c.PERM_MANU_CHANGE_M, instance)
            and (
                (
                    instance._status == instance.Status.NEW
    ...
    @transition(
        ...
        conditions=[],
        permission=lambda instance, user: (
            (instance._status == instance.Status.NEW and user.has_any_perm(c.PERM_MANU_CHANGE_M, instance))
            or (instance._status != instance.Status.NEW and user.has_any_perm(c.PERM_MANU_VIEW_M, instance))
        ),
    )
    def view_noop(self):
        return self._status
    ...
    @transition(
        field=_status,
        source=[Status.PENDING_DATAVERSE_PUBLISH, Status.PUBLISHED_TO_DATAVERSE, Status.COMPLETED_REPORT_SENT],
        target=RETURN_VALUE(),
        conditions=[can_dataverse_upload_noop],
    ...
    @transition(
        ...
        source=[Status.PENDING_DATAVERSE_PUBLISH, Status.PUBLISHED_TO_DATAVERSE, Status.COMPLETED_REPORT_SENT],
        target=RETURN_VALUE(),
        conditions=[can_dataverse_pull_citation],
        permission=lambda instance, user: user.has_any_perm(c.PERM_MANU_CURATE, instance),
    )
...
class LocalContainerInfo(models.Model):
    ...
    def container_public_address(self):
        ...
        return (
            settings.CONTAINER_PROTOCOL + "://" + self.proxy_container_address + ":" + str(proxy_container_external_port)
        )  # I don't understand why python decides my charfield is an int?
...
class GitFile(AbstractCreateUpdateModel):
    ...
    path = models.CharField(
        max_length=4096, verbose_name="file path", help_text="The path of the folders holding the file, not including the filename"
    )
    ...
    class Meta:
        constraints = [models.UniqueConstraint(fields=["path", "name", "parent_submission"], name="GitFile submission and path")]
        indexes = [
...
class Note(AbstractCreateUpdateModel):
        ...
        if not self.pk:
            ...
            if self.parent_submission._status == Submission.Status.NEW or self.parent_submission._status == Submission.Status.REJECTED_EDITOR:
                ...
                if local.user.groups.filter(name=c.GROUP_MANUSCRIPT_CURATOR_PREFIX + " " + str(self.manuscript.id)).exists():
                    ...
                    self.ref_cycle = (
                        self.RefCycle.VERIFICATION
                    )  # If during verification assigned as either curator or verifier we default to verifier
            elif (
                self.parent_submission._status == Submission.Status.REVIEWED_AWAITING_REPORT
                or self.parent_submission._status == Submission.Status.REVIEWED_REPORT_AWAITING_APPROVAL
    ...
    def is_private(self, checkers=None):
        ...
        if (
            role_count == 4
        ):  # pretty crude check, if all roles then its public. Using a magic number (4) instead of len(c.get_roles()) because its already hardcoded other places.
            return False
        else:
            return True
...
class CorereInvitation(Invitation):
    ...
    def create(cls, email, user, inviter=None, **kwargs):
        ...
        instance = cls._default_manager.create(email=email, user=user, key=key, inviter=inviter, **kwargs)
        return instance
    ...
    def send_invitation(self, request, extra_text="", **kwargs):
        ...
        signals.invite_url_sent.send(sender=self.__class__, instance=self, invite_url_sent=invite_url, inviter=self.inviter)
...
class DataverseInstallation(models.Model):
    ...
    username = models.CharField(
        ...
        help_text="User that owns the API token. We store this only for informational purposes.",
    )
