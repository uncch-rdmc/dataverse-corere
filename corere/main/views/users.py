...
def invite_assign_author(request, id=None):
        ...
        if form.is_valid():
            ...
            for u in users:
                if not u.groups.filter(name=c.GROUP_ROLE_AUTHOR).exists():
                    logger.warn(
                        "User {0} attempted to add user id {1} from group {2} when they don't have the base role (probably by hacking the form)".format(
                            request.user.id, u.id, group_substring
                        )
                    )
                    raise Http404()
    ...
    return render(
        ...
        "main/form_assign_user.html",
        {
            ...
            "role_name": "Author",
            "assigned_users": manu_author_group.user_set.all(),
            "can_remove_author": can_remove_author,
...
@permission_required_or_404(
    c.perm_path(c.PERM_MANU_ADD_AUTHORS), (m.Manuscript, "id", "id"), accept_global_perms=True
)  # slightly hacky that you need add to access the remove function, but everyone with remove should be able to add
@require_http_methods(["GET", "POST"])
...
def assign_editor(request, id=None):
    ...
    return render(
        ...
        "main/form_assign_user.html",
        {
            "form": form,
            "id": id,
            "select_table_info": helper_generate_select_table_info(c.GROUP_ROLE_EDITOR, group_substring),
            "group_substring": group_substring,
            "role_name": "Editor",
            "assigned_users": manu_editor_group.user_set.all(),
            "manuscript_display_name": manuscript.get_display_name(),
            "page_title": page_title,
...
def assign_curator(request, id=None):
    ...
    return render(
        ...
        "main/form_assign_user.html",
        {
            "form": form,
            "id": id,
            "select_table_info": helper_generate_select_table_info(c.GROUP_ROLE_CURATOR, group_substring),
            "group_substring": group_substring,
            "role_name": "Curator",
            "assigned_users": manu_curator_group.user_set.all(),
            "manuscript_display_name": manuscript.get_display_name(),
            "page_title": page_title,
            "helper": f.StandardUserAddFormHelper(),
        },
...
def assign_verifier(request, id=None):
    ...
    return render(
        ...
        "main/form_assign_user.html",
        {
            "form": form,
            "id": id,
            "select_table_info": helper_generate_select_table_info(c.GROUP_ROLE_VERIFIER, group_substring),
            "group_substring": group_substring,
            "role_name": "Verifier",
            "assigned_users": manu_verifier_group.user_set.all(),
            "manuscript_display_name": manuscript.get_display_name(),
            "page_title": page_title,
            "helper": f.StandardUserAddFormHelper(),
        },
...
def account_complete_oauth(request):
    if settings.CONTAINER_DRIVER == "wholetale":
        ...
        r = requests.get(
            ...
            params={
                "redirect": protocol + "://" + settings.SERVER_ADDRESS + "/account_user_details/?girderToken={girderToken}"
        ...
        try:  # request.user.invite will error if there is no invite
            ...
            if settings.CONTAINER_DRIVER == "wholetale":
                ...
                for group in request.user.groups.all():
                    if (
                        group.name.startswith(c.GROUP_MANUSCRIPT_EDITOR_PREFIX)
...
def notifications(request):
    return render(request, "main/notifications.html", {"page_title": _("notifications_pageTitle")})
        ...
        if request.method == "POST":
            if form.is_valid():
                ...
                if m.User.objects.filter(email=email):
                    ...
                    return render(
                        request,
                        "main/form_user_details.html",
                        {"form": form, "helper": helper, "page_title": "Invite {0}".format(role_text.capitalize())},
                    )
                ...
                logger.debug(form.errors)  # TODO: DO MORE?
        return render(
            request, "main/form_user_details.html", {"form": form, "helper": helper, "page_title": "Invite {0}".format(role_text.capitalize())}
        )
