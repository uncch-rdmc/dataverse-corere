...
class GenericCorereObjectView(View):
    ...
    def get(self, request, *args, **kwargs):
        ...
        context = {
            ...
            "obj_type": self.object_friendly_name,
            "create": self.create,
...
class GenericManuscriptView(GenericCorereObjectView):
    ...
    def general(self, request, *args, **kwargs):
        ...
        if (
            not self.object.has_submissions() and self.role_name == "Editor"
        ):  # we need a different form/helper for editor during create to hide certain fields
    ...
    def get(self, request, *args, **kwargs):
        ...
        context = {
            ...
            "obj_type": self.object_friendly_name,
            "create": self.create,
            "from_submission": self.from_submission,
            "m_status": self.object._status,
            "page_title": self.page_title,
            "page_help_text": self.page_help_text,
    ...
    def post(self, request, *args, **kwargs):
        ...
        if (
            ...
            and (not self.author_formset or self.author_formset.is_valid())
            and (not self.data_source_formset or self.data_source_formset.is_valid())
    ...
    def get(self, request, *args, **kwargs):
        ...
        return render(
            ...
            self.template,
            {
                ...
                "manuscript_id": self.object.id,
                "manuscript_display_name": self.object.get_display_name(),
                "files_dict_list": list(self.files_dict_list),
                "file_url_base": self.file_url_base,
    ...
    def post(self, request, *args, **kwargs):
        ...
        if not self.read_only:
            ...
            context = {
                ...
                "m_status": self.object._status,
                "manuscript_display_name": self.object.get_display_name(),
                "files_dict_list": list(self.object.get_gitfiles_pathname(combine=True)),
                "file_url_base": self.file_url_base,
                "progress_bar_html": progress_bar_html,
                "obj_id": self.object.id,
                "obj_type": self.object_friendly_name,
    ...
    def get(self, request, *args, **kwargs):
        ...
        return render(
            ...
            self.template,
            {
                ...
                "read_only": self.read_only,
                "manuscript_display_name": self.object.get_display_name(),
                "files_dict_list": self.files_dict_list,
                "manuscript_id": self.object.id,
                "obj_id": self.object.id,
                "obj_type": self.object_friendly_name,
                "repo_branch": "master",
...
class ManuscriptPullCitationFromDataverseView(LoginRequiredMixin, GetOrGenerateObjectMixin, TransitionPermissionMixin, GenericManuscriptView):
    http_method_names = ["get"]
    transition_method_name = (
        "dataverse_pull_citation"  # TODO: is it ok to call as non noop transition here? If so, remove comment in TransitionPermissionMixin
    )
    model = m.Manuscript
    object_friendly_name = "manuscript"
    ...
    def get(self, request, *args, **kwargs):
        ...
        try:
            ...
            self.msg = (
                ...
                + self.object.dataverse_fetched_doi
                + ' has been fetched. Information can be confirmed by viewing the <a href="./reportdownload">verification report</a>.'
            )
            list(messages.get_messages(request))  # Clears messages if there are any already.
...
# TODO: Because we are calling this without permissionrequiredmixin due to our complex logic, we need to improve how early we kick out users
...
class GenericSubmissionFormView(GenericCorereObjectView):
    ...
    # We use the "selected" ones to keep the uninitialized formset stored for our re-init trick to hide errors.
    # ... I think this trick also handles some weird duplication that was happening with notes when another thing errored.
    selected_formset_author = f.NoteSubmissionFormsetOutOfPhase
    selected_formset_editor = f.NoteSubmissionFormsetOutOfPhase
    selected_formset_curator = f.NoteSubmissionFormsetOutOfPhase
    selected_formset_verifier = f.NoteSubmissionFormsetOutOfPhase
    ...
    def general(self, request, *args, **kwargs):
        ...
        try:
            if not self.read_only and (
                ...
                or has_transition_perm(self.object.submission_edition.edit_noop, request.user)
            ):
        ...
        try:
            if not self.read_only and (
                ...
                or has_transition_perm(self.object.submission_curation.edit_noop, request.user)
            ):
        ...
        try:
            if not self.read_only and (
                ...
                or has_transition_perm(self.object.submission_verification.edit_noop, request.user)
            ):
                if (
    ...
    def get(self, request, *args, **kwargs):
        ...
        context = {
            ...
            "obj_type": self.object_friendly_name,
            "create": self.create,
            "inline_helper": f.GenericInlineFormSetHelper(),
            "s_version": self.object.version_id,
            "page_title": self.page_title,
            "page_help_text": self.page_help_text,
            "manuscript_display_name": manuscript_display_name,
        ...
        if self.submission_editor_date_formset is not None:
            context["submission_editor_date_formset"] = self.submission_editor_date_formset(
                queryset=m.Submission.objects.filter(id=self.object.id), prefix="submission_editor_date_formset"
            )
    ...
    def post(self, request, *args, **kwargs):
        ...
        if self.submission_editor_date_formset:
            self.submission_editor_date_formset = self.submission_editor_date_formset(
                request.POST, queryset=m.Submission.objects.filter(id=self.object.id), prefix="submission_editor_date_formset"
            )
        ...
        if not self.read_only:
            ...
            if not self.read_only and (
                has_transition_perm(self.object.add_curation_noop, request.user)
                or (hasattr(self.object, "submission_curation") and has_transition_perm(self.object.submission_curation.edit_noop, request.user))
            ):
            ...
            if (
                ...
                if self.submission_editor_date_formset:
                    ...
                    self.object = self.model.objects.get(
                        id=o_id
                    )  # We re-fetch the object here to ensure that the saves below don't wipe out the date. Not quite sure why this is needed
                if self.edition_formset:
                ...
                try:
                    ...
                    if request.POST.get("submit_progress_edition"):
                        ...
                        ### Messaging - Display banner message that the review is completed ###
                        ...
                        ### Messaging - Display banner message that the review is completed ###
                        if status == m.Submission.Status.IN_PROGRESS_VERIFICATION:
                            role_text = "verifier"
                            self.msg = _("submission_reviewSubmitted_banner").format(role_text=role_text)
                        elif self.object.manuscript._status == m.Manuscript.Status.PENDING_DATAVERSE_PUBLISH:
                            # No verification needed, submission accepted by curator
                        ...
                        if self.verification_formset and self.verification_formset.is_valid():
                            ...
                            ### Messaging - Display banner message that the review is completed ###
                            self.msg = _("submission_reviewSubmitted_banner").format(role_text="curator")
                            list(messages.get_messages(request))  # Clears messages if there are any already.
                            messages.add_message(request, messages.SUCCESS, self.msg)
                    ...
                    if status != None:
                        if status == m.Submission.Status.IN_PROGRESS_CURATION:
                            ...
                            notification_msg = _("submission_objectReviewed_notification_forCurator").format(
                                object_id=self.object.manuscript.id,
                                object_title=self.object.manuscript.get_display_name(),
                                object_url=self.object.manuscript.get_landing_url(request),
                            )
...
#TODO-NOTE: re-init note formsets here to hide validation errors? (See 1548)

        ### NOTES ###

        #TODO: Remove unused code below. We don't need it in post.
        ...
        if self.note_formset_author is not None:
            ...
            for checker in checkers:
                checker.prefetch_perms(notes)
            sub_files = self.object.submission_files.all().order_by("path", "name")
            context["note_formset_author"] = self.selected_formset_author(
                instance=self.object,
                queryset=m.Note.objects.filter(parent_submission=self.object, ref_cycle=m.Note.RefCycle.SUBMISSION),
                prefix="note_formset_author",
                form_kwargs={"checkers": checkers, "manuscript": self.object.manuscript, "submission": self.object, "sub_files": sub_files},
        ...
        if self.note_formset_editor is not None:
            ...
            if sub_files is None:
                sub_files = self.object.submission_files.all().order_by("path", "name")
            context["note_formset_editor"] = self.selected_formset_editor(
                instance=self.object,
                queryset=m.Note.objects.filter(parent_submission=self.object, ref_cycle=m.Note.RefCycle.EDITION),
                prefix="note_formset_editor",
                form_kwargs={"checkers": checkers, "manuscript": self.object.manuscript, "submission": self.object, "sub_files": sub_files},
        ...
        if self.note_formset_curator is not None:
            ...
            if sub_files is None:
                sub_files = self.object.submission_files.all().order_by("path", "name")
            context["note_formset_curator"] = self.selected_formset_curator(
                instance=self.object,
                queryset=m.Note.objects.filter(parent_submission=self.object, ref_cycle=m.Note.RefCycle.CURATION),
                prefix="note_formset_curator",
                form_kwargs={"checkers": checkers, "manuscript": self.object.manuscript, "submission": self.object, "sub_files": sub_files},
        ...
        if self.note_formset_verifier is not None:
            ...
            if sub_files is None:
                sub_files = self.object.submission_files.all().order_by("path", "name")
            context["note_formset_verifier"] = self.selected_formset_verifier(
                instance=self.object,
                queryset=m.Note.objects.filter(parent_submission=self.object, ref_cycle=m.Note.RefCycle.VERIFICATION),
                prefix="note_formset_verifier",
                form_kwargs={"checkers": checkers, "manuscript": self.object.manuscript, "submission": self.object, "sub_files": sub_files},
...
class SubmissionUploadFilesView(LoginRequiredMixin, GetOrGenerateObjectMixin, TransitionPermissionMixin, GitFilesMixin, GenericCorereObjectView):
    ...
    def general(self, request, *args, **kwargs):
        if self.read_only:
            ...
            self.page_help_text = _("submission_completeFiles_helpText").format(
                dataverse=self.object.manuscript.dataverse_parent, installation=self.object.manuscript.dataverse_installation.name
            )
        else:
    ...
    def get(self, request, *args, **kwargs):
        ...
        context = {
            ...
            "page_title": self.page_title,
            "page_help_text": self.page_help_text,
            "dataverse_upload": self.dataverse_upload,
            "skip_docker": settings.SKIP_DOCKER,
            "containerized": self.object.manuscript.is_containerized(),
            "manuscript_id": self.object.manuscript.id,
        }
        ...
        if not self.read_only:
            ...
            if not errors and request.POST.get("submit_dataverse_upload"):
                ...
                try:
                    ...
                    if (
                        old_doi and old_doi != self.object.manuscript.dataverse_fetched_doi
                    ):  # I don't actually know why I'm checking doi equality here... we could probably just check old_doi existing
                        self.msg = (
                            'You have uploaded the manuscript, which created a new dataset. You may want to go to <a href="'
                            + old_dv_url
                            + "/dataset.xhtml?persistentId="
                            + old_doi
                            + '">'
                            + old_doi
                            + "</a> and delete the previous dataset."
                        )
                        list(messages.get_messages(request))  # Clears messages if there are any already.
                        messages.add_message(request, messages.SUCCESS, mark_safe(self.msg))
                    else:
                    ...
                    messages.add_message(request, messages.ERROR, self.msg)
                    logger.error(
                        "An error has occurred attempting to upload manuscript {} to Dataverse: {}".format(str(self.object.manuscript.id), str(e))
                    )
...
class SubmissionFinishView(LoginRequiredMixin, GetOrGenerateObjectMixin, GenericCorereObjectView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            ...
            try:
                ...
                if self.object._status == m.Submission.Status.RETURNED:
                    if self.object.manuscript._status == m.Manuscript.Status.PENDING_DATAVERSE_PUBLISH:
                        ...
                        self.msg = _("submission_objectComplete_banner").format(
                            manuscript_id=self.object.manuscript.id, manuscript_display_name=self.object.manuscript.get_display_name()
                        )
                        list(messages.get_messages(request))  # Clears messages if there are any already.
                        messages.add_message(request, messages.SUCCESS, self.msg)
                        recipients = m.User.objects.filter(
                            groups__name__startswith=c.GROUP_MANUSCRIPT_AUTHOR_PREFIX + " " + str(self.object.manuscript.id)
                        )
                        notification_msg = _("manuscript_update_notification_forAuthor").format(
                            object_id=self.object.manuscript.id,
                            object_title=self.object.manuscript.get_display_name(),
                            object_url=self.object.manuscript.get_landing_url(request),
                        )
                        ...
                        self.msg = _("submission_objectTransferAuthorSuccess_banner").format(
                            manuscript_id=self.object.manuscript.id, manuscript_display_name=self.object.manuscript.get_display_name()
                        )
                        list(messages.get_messages(request))  # Clears messages if there are any already.
                        messages.add_message(request, messages.SUCCESS, self.msg)
                        ...
                        notification_msg = _("manuscript_objectTransferAuthor_notification_forAuthor").format(
                            object_id=self.object.manuscript.id,
                            object_title=self.object.manuscript.get_display_name(),
                            object_url=self.object.manuscript.get_landing_url(request),
                        )
            ...
            self.msg = _("submission_objectTransferAuthorFailure_banner").format(
                manuscript_id=self.object.manuscript.id, manuscript_display_name=self.object.manuscript.get_display_name()
            )
            list(messages.get_messages(request))  # Clears messages if there are any already.
            messages.add_message(request, messages.ERROR, self.msg)
...
class SubmissionNotebookRedirectView(LoginRequiredMixin, GetOrGenerateObjectMixin, TransitionPermissionMixin, GenericCorereObjectView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if "postauth" in request.GET:
            request.user.last_oauthproxy_forced_signin = (
                datetime.now()
            )  # maybe change to timezone.now() https://stackoverflow.com/questions/18622007/
            request.user.save()
...
class SubmissionNotebookView(LoginRequiredMixin, GetOrGenerateObjectMixin, GenericCorereObjectView):
    ...
    def general(self, request, *args, **kwargs):
        if not self.object.manuscript.is_containerized():
            ...
            try:
                self.wtm_tale = self.dominant_group.groupconnector_tales.get(
                    submission=self.object
                )  # this will blow up and 404 if there is no dominant group, which we want
                ...
                if self.object.version_id < self.object.manuscript.get_max_submission_version_id():
                    if not wtc_tale_target_version:
                        raise Http404(
                            "The compute environment cannot be launched for this submission, because the compute environment changed after this submission was created."
                        )
                    wtc_versioned_tale = wtc.restore_tale_to_version(wtc_versioned_tale["_id"], wtc_tale_target_version["_id"])
                self.wtm_tale = wtm.Tale.objects.create(
                    manuscript=self.object.manuscript,
                    submission=self.object,
                    wt_id=wtc_versioned_tale["_id"],
                    group_connector=self.dominant_group,
                    original_tale=wtm_parent_tale,
                )
    ...
    def get(self, request, *args, **kwargs):
        ...
        context = {
            ...
            "manuscript_display_name": self.object.manuscript.get_display_name(),
            "manuscript_id": self.object.manuscript.id,
            "skip_edition": self.object.manuscript.skip_edition,
        }
...
class SubmissionDownloadWholeTaleNotebookView(LoginRequiredMixin, GetOrGenerateObjectMixin, SubmissionGenericWholeTalePermissionView):
    ...
    def get(self, request, *args, **kwargs):
        self.general(request, *args, **kwargs)
        if (
            not self.object.manuscript.is_containerized() or settings.CONTAINER_DRIVER != "wholetale"
        ):  # We don't check compute_env here because it'll be handled by dispatch
            return Http404()
        ...
        response["Content-Disposition"] = (
            'attachment; filename="'
            + self.object.manuscript.slug
            + "_-_submission_"
            + str(self.object.version_id)
            + "_-_whole_tale_contents_"
            + self.dominant_group.corere_group.name.split()[0]
            + '.zip"'
        )
        return response
...
def _helper_generate_whole_tale_stream_contents(wtc, submission, user, girderToken):
    ...
    for event in client.events():
        ...
        if data["type"] == "wt_progress":
            ...
            msg = "  -> event received:" f" msg = {data['data']['message']}" f" status = {data['data']['state']}" f" progress = {progress}%"
            yield (msg + "<br>")
            if progress == 100:
...
def _helper_get_oauth_url(request, submission):
    ...
    if request.user.last_oauthproxy_forced_signin + timedelta(days=1) < timezone.now():
        ...
        container_flow_address = (
            submission.manuscript.manuscript_localcontainerinfo.container_public_address() + "/submission/" + str(submission.id) + "/notebooklogin/"
        )
    ...
    if submission._status == submission.Status.NEW or submission._status == submission.Status.REJECTED_EDITOR:
        ...
        if submission.manuscript.is_containerized() and settings.CONTAINER_DRIVER == "wholetale":
            ...
            try:  # Get tale if it exists. This happens when a submission was returned by an editor, as this does not create a new submission
                ...
                tale_copy_editor = wtm.Tale.objects.create(
                    manuscript=submission.manuscript,
                    submission=submission,
                    wt_id=wtc_tale_copy_editor["_id"],
                    group_connector=group_editor.wholetale_group,
                    original_tale=tale_original,
                )
            ...
            try:
                ...
                tale_copy_curator = wtm.Tale.objects.create(
                    manuscript=submission.manuscript,
                    submission=submission,
                    wt_id=wtc_tale_copy_curator["_id"],
                    group_connector=group_curator.wholetale_group,
                    original_tale=tale_original,
                )
            ...
            try:
                ...
                tale_copy_verifier = wtm.Tale.objects.create(
                    manuscript=submission.manuscript,
                    submission=submission,
                    wt_id=wtc_tale_copy_verifier["_id"],
                    group_connector=group_verifier.wholetale_group,
                    original_tale=tale_original,
                )
