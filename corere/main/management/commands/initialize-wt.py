...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            "--createlocal",
            action="store_true",
            help="Create the local wholetale.admin group connected to what is created in the Whole Tale instance.",
        )
        parser.add_argument(
            "--deletegroups", action="store_true", help="Deletes all remote/local groups. Note that doing this will break existing access"
        )
        parser.add_argument(
            "--deletetales", action="store_true", help="Deletes all remote/local tales. Note that doing this will break existing manuscripts."
        )
    ...
    def handle(self, *args, **options):
        ...
        if deletegroups:
            for wtc_group in wtc.get_all_groups():
                if (
                    wtc_group["name"].startswith(c.GROUP_MANUSCRIPT_AUTHOR_PREFIX)
                    or wtc_group["name"].startswith(c.GROUP_MANUSCRIPT_EDITOR_PREFIX)
                    or wtc_group["name"].startswith(c.GROUP_MANUSCRIPT_CURATOR_PREFIX)
                    or wtc_group["name"].startswith(c.GROUP_MANUSCRIPT_VERIFIER_PREFIX)
