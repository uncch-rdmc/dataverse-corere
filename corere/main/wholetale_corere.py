...
class WholeTaleCorere(WholeTale):
        ...
        try:
            ...
            if e.response.status_code == 400 and json.loads(e.responseText)["message"].startswith("You have reached a limit for running instances"):
                ...
                try:
                    ...
                    if e.response.status_code == 400 and json.loads(e.responseText)["message"].startswith(
                        ...
                        other_submission_instances = wtm.Instance.objects.filter(
                            Q(corere_user=user) & Q(tale__manuscript=wtm_tale.manuscript) & ~Q(tale__submission=wtm_tale.submission)
                        )
                        ...
                        try:
                            ...
                            if e.response.status_code == 400 and json.loads(e.responseText)["message"].startswith(
                                ...
                                raise Exception(
                                    f"Your maximum number of instances in Whole Tale has been reached. Some of your running instances are not managed by CORE2 so they cannot be deleted. Please go to {settings.WHOLETALE_BASE_URL} and delete running instances manually before proceeding."
                                )
    ...
    def set_group_access(self, tale_id, level, wtm_group, force_instance_shutdown=True):
        ...
        if level != self.AccessType.NONE:  # If access is none, we need to not add it, instead of setting level as NONE (-1)
            acl = {"id": wtm_group.wt_id, "name": wtm_group.corere_group.name, "flags": [], "level": level}
